...
function loadSolvedProgress() {
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(arr)) {
        arr.forEach((id) => {
          ...
          if (PROBLEMS && PROBLEMS[cleanId]) {
            solvedProblems.add(cleanId);
          } else if (PROBLEMS) {
            const match = Object.values(PROBLEMS).find(
              (p) =>
                p &&
                (String(p.num) === cleanId ||
                  p.id === cleanId ||
                  String(p.num) === String(parseInt(cleanId, 10))),
            );
            if (match) {
              solvedProblems.add(match.id);
            }
          }
        });
        saveSolvedProgress();
    ...
    console.warn("Could not load solved problems from storage:", e);
  }
}
...
function initCodeMirror() {
  ...
  try {
    const curProb = typeof PROBLEMS !== "undefined" && currentProblemId ? PROBLEMS[currentProblemId] : null;
    const initialMode = curProb && curProb.language === "sql" ? "text/x-sql" : "text/x-java";
    ...
    // Initialize custom hardware-accelerated Smooth Caret
    setupSmoothCaret(cmEditor);
    ...
    if (CodeMirror.registerHelper) {
      ...
      CodeMirror.registerHelper("hint", "sql", function (editor, options) {
        ...
        if (wLower.length > 0) {
          if (typeof SQL_SNIPPETS !== "undefined") {
            Object.entries(SQL_SNIPPETS).forEach(([key, snip]) => {
              if (key.startsWith(wLower)) {
                list.push({
                  text: snip.body,
                  displayText: `${snip.label} → ${snip.desc}`,
                  className: "cm-hint-snippet",
                  ...
                  hint: function (cm) {
                    ...
                    if (snip.body.includes("\n")) {
                      ...
                      cm.setCursor({
                        line: from.line,
                        ch: from.ch + (snip.cursorOffset || snip.body.length),
                      });
                    }
                  },
                });
          ...
          if (typeof SQL_KEYWORDS !== "undefined") {
            SQL_KEYWORDS.forEach((kw) => {
              if (kw.toLowerCase().startsWith(wLower) && kw.toLowerCase() !== wLower) {
                list.push({
                  text: kw + " ",
                  displayText: kw,
                  ...
                  render: function (elt) {
                    ...
                    const icon = document.createElement("span");
                    icon.className = "cm-hint-icon cm-hint-icon-word";
                    icon.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><ellipse cx="12" cy="5" rx="9" ry="3"></ellipse><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"></path><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"></path></svg>`;
                    const name = document.createElement("span");
                    name.className = "cm-hint-name";
                    name.textContent = kw;
                    left.appendChild(icon);
    ...
    cmEditor.on("inputRead", (cm, change) => {
      ...
      if (/[a-zA-Z\._]/.test(text) && !cm.state.completionActive) {
        const prob = typeof PROBLEMS !== "undefined" && currentProblemId ? PROBLEMS[currentProblemId] : null;
        const isSql = prob && prob.language === "sql";
        const hintHelper = isSql
          ? (CodeMirror.hint.sql || CodeMirror.hint.anyword)
          : (CodeMirror.hint.java || CodeMirror.hint.anyword);
...
// ==========================================
// VS Code-style Smooth Caret Animation System
// ==========================================
let smoothCaretEl = null;
let caretMovingTimeout = null;

function setupSmoothCaret(cm) {
  if (!cm) return;

  const wrapper = cm.getWrapperElement();
  const linesEl = wrapper.querySelector(".CodeMirror-lines");
  if (!linesEl) return;

  const oldCaret = wrapper.querySelector(".cm-smooth-caret");
  if (oldCaret) oldCaret.remove();

  smoothCaretEl = document.createElement("div");
  smoothCaretEl.className = "cm-smooth-caret cm-caret-hidden";
  smoothCaretEl.setAttribute("aria-hidden", "true");
  linesEl.appendChild(smoothCaretEl);

  function updateCaretPosition(immediate = false) {
    if (!cm || !smoothCaretEl) return;

    if (!cm.hasFocus()) {
      smoothCaretEl.classList.add("cm-caret-hidden");
      return;
    }
    ...
    const height = Math.max(16, (coords.bottom || coords.top + 20) - coords.top);

    smoothCaretEl.classList.remove("cm-caret-hidden");

    if (immediate) {
      smoothCaretEl.classList.add("no-transition");
      smoothCaretEl.style.transform = `translate3d(${coords.left}px, ${coords.top}px, 0)`;
      smoothCaretEl.style.height = `${height}px`;
      void smoothCaretEl.offsetHeight; // Force layout reflow
      smoothCaretEl.classList.remove("no-transition");
    } else {
      smoothCaretEl.style.transform = `translate3d(${coords.left}px, ${coords.top}px, 0)`;
      smoothCaretEl.style.height = `${height}px`;
    }

    // Keep solid and fully visible during motion
    smoothCaretEl.classList.add("cm-caret-moving");
    clearTimeout(caretMovingTimeout);
    caretMovingTimeout = setTimeout(() => {
      if (smoothCaretEl) {
        smoothCaretEl.classList.remove("cm-caret-moving");
      }
    }, 450);
  }
  ...
  cm.on("focus", () => updateCaretPosition(true));
  cm.on("blur", () => {
    if (smoothCaretEl) smoothCaretEl.classList.add("cm-caret-hidden");
  });
  cm.on("refresh", () => updateCaretPosition(true));
  cm.on("scroll", () => updateCaretPosition(true));

  if (cm.hasFocus()) {
    setTimeout(() => updateCaretPosition(true), 60);
  }
}

// Toggle line comments (// for Java, -- for SQL) for selected lines or current line in CodeMirror
function toggleComment(cm) {
  if (!cm) return;
  const prob = typeof PROBLEMS !== "undefined" && currentProblemId ? PROBLEMS[currentProblemId] : null;
  const isSql = prob && prob.language === "sql";
  const commentToken = isSql ? "-- " : "// ";
  const commentRegex = isSql ? /^(\s*)--\s?/ : /^(\s*)\/\/\s?/;
  const isLineCommented = (str) => isSql ? str.startsWith("--") : str.startsWith("//");
  ...
  cm.operation(() => {
    ...
    // Determine if all non-empty lines in selection are already commented
    ...
    if (nonBlankCount === 0) {
      // Empty line, just insert comment
      cm.replaceRange(
        commentToken,
        { line: startLine, ch: 0 },
        { line: startLine, ch: 0 },
    ...
    if (allCommented) {
      // Uncomment each line: remove first occurrence of comment token
      for (let i = startLine; i <= endLine; i++) {
        const lineText = cm.getLine(i);
      ...
      // Comment each line: add comment token after indentation
      for (let i = startLine; i <= endLine; i++) {
        const lineText = cm.getLine(i);
...
function renderProblemNavList() {
  ...
  // Filter problems based on filter pills and search query
  ...
  // Group filtered problems by category with ST2 Questions and Sample ETE at the top
  const categoryGroups = new Map();
  
  // 1. First add ST2 Questions and Sample ETE if present in filtered problems
  ["ST2 Questions", "Sample ETE"].forEach((topCat) => {
    const matchingProbs = filteredProblems.filter((p) => (p.category || "Java Practice") === topCat);
    if (matchingProbs.length > 0) {
      categoryGroups.set(topCat, matchingProbs);
    }
  });

  // 2. Then add all other curriculum categories in their natural order
  filteredProblems.forEach((p) => {
    const cat = p.category || "Java Practice";
    if (cat === "ST2 Questions" || cat === "Sample ETE") return;
  ...
  const bankLabel = document.getElementById("sidebar-bank-label");
  if (bankLabel) {
    bankLabel.textContent = "Curriculum Bank";
  }
...
const SQL_SNIPPETS = {
  ...
  selw: {
    label: "selw",
    desc: "SELECT * FROM table WHERE condition",
    body: "SELECT * FROM  WHERE ",
    cursorOffset: 14,
  },
  grp: {
    label: "grp",
  ...
  join: {
    label: "join",
    desc: "INNER JOIN table ON condition",
    body: "INNER JOIN  ON ",
    cursorOffset: 11,
  },
  left: {
    label: "left",
    desc: "LEFT JOIN table ON condition",
    body: "LEFT JOIN  ON ",
    cursorOffset: 10,
  },
  upd: {
    label: "upd",
    desc: "UPDATE table SET col = val WHERE condition",
    body: "UPDATE  SET  WHERE ",
    cursorOffset: 7,
  },
  del: {
    label: "del",
    desc: "DELETE FROM table WHERE condition",
    body: "DELETE FROM  WHERE ",
    cursorOffset: 12,
  },
  cre: {
    label: "cre",
...
const SQL_KEYWORDS = [
  ...
  "PRIMARY KEY", "FOREIGN KEY", "NOT NULL", "UNIQUE", "CHECK", "DEFAULT", "AUTO_INCREMENT",
  "AND", "OR", "NOT", "IN", "BETWEEN", "LIKE", "IS NULL", "IS NOT NULL",
  "AS", "DISTINCT", "EXISTS", "CASE", "WHEN", "THEN", "ELSE", "END",
  "ASC", "DESC", "COUNT", "SUM", "AVG", "MIN", "MAX", "ROUND",
  "INT", "INTEGER", "VARCHAR", "TEXT", "DECIMAL", "NUMERIC", "FLOAT", "DOUBLE", "DATE", "DATETIME"
];
...
function tryExpandSnippet(cm) {
  ...
  const prob = typeof PROBLEMS !== "undefined" && currentProblemId ? PROBLEMS[currentProblemId] : null;
  const isSql = prob && prob.language === "sql";
  const snippets = isSql ? SQL_SNIPPETS : JAVA_SNIPPETS;
  ...
  if (snippet) {
    ...
    if (snippet.body.includes("\n")) {
      ...
      cm.setCursor({ line: from.line, ch: from.ch + (snippet.cursorOffset || snippet.body.length) });
    }
    return true;
  }
  return false;
...
function updateProblemView() {
  ...
  document.getElementById("solution-badge-text").textContent =
    prob.category || prob.tag || (isSql ? "SQL Query" : "Java Solution");

  const editorFileName = document.getElementById("editor-file-name");
  if (editorFileName) {
    editorFileName.textContent = isSql ? "query.sql" : "Main.java";
  }
  const editorLangTag = document.getElementById("editor-lang-tag");
  if (editorLangTag) {
    editorLangTag.textContent = isSql ? "MySQL / SQL" : "Java 21";
  }
  const compilerStatusText = document.getElementById("compiler-status-text");
  if (compilerStatusText) {
    compilerStatusText.textContent = isSql ? "MySQL Engine Ready" : "Java 21 Ready (Local)";
  }
  const compilerStatusBadge = document.getElementById("compiler-status-badge");
  if (compilerStatusBadge) {
    compilerStatusBadge.title = isSql ? "MySQL / SQL Execution Engine Ready" : "Java Compiler Readiness";
  }
  const solTitleText = document.getElementById("solution-title-text");
  if (solTitleText) {
    solTitleText.textContent = isSql ? "Reference SQL Solution" : "Reference Java Solution";
  }
  const solCardBadge = document.getElementById("sol-card-badge-text");
  if (solCardBadge) {
    solCardBadge.textContent = isSql ? "Reference SQL Query" : "Reference Java 21 Code";
  }
  const formatBtn = document.getElementById("btn-format-code");
  if (formatBtn) {
    formatBtn.title = isSql ? "Auto Format SQL Query (Alt+F)" : "Auto Format / Indent Java Code (Alt+F)";
  }
  if (codeTextarea) {
    codeTextarea.placeholder = isSql ? "Write your SQL query here..." : "Write your Java code here...";
  }
  if (cmEditor) {
    const targetMode = isSql ? "text/x-sql" : "text/x-java";
    if (cmEditor.getOption("mode") !== targetMode) {
      cmEditor.setOption("mode", targetMode);
    }
  }
  ...
  if (solPreview) {
    solPreview.innerHTML = `<code>${escapeHtml(solutionText || (isSql ? "-- No SQL solution available" : "// No solution available"))}</code>`;
  }
...
function switchProblem(probId, updateHistory = true) {
  // Guarantee existing editor code is saved for the current question before switching
  saveCodeToStorage();
  if (typeof editorDiagnostics !== "undefined") {
    editorDiagnostics.clear();
  }
...
function loadSavedCodeOrBoilerplate() {
  ...
  try {
    ...
    if (saved !== null && saved !== undefined) {
      // If user had previous leaked solution cached for SQL question, clean it up and load fresh starter
      if (prob.language === "sql" && prob.solutionCode && saved.trim() === prob.solutionCode.trim()) {
...
function setupEventListeners() {
  ...
  if (btnFormatCode) {
    btnFormatCode.addEventListener("click", () => {
      ...
      const prob = typeof PROBLEMS !== "undefined" && currentProblemId ? PROBLEMS[currentProblemId] : null;
      const isSql = prob && prob.language === "sql";
      showToast(isSql ? "SQL query formatted" : "Java code formatted & indented");
    });
  }
  ...
  // Guarantee all code, solved state, and timers are immediately saved on tab switch / window close
  const persistAllProgress = () => {
    saveCodeToStorage();
    saveActiveProblem(currentProblemId);
    saveSolvedProgress();
    saveStarredProgress();
    if (typeof practiceTimer !== "undefined" && practiceTimer.saveTimes) {
      practiceTimer.saveTimes();
    }
  };

  window.addEventListener("beforeunload", persistAllProgress);
  window.addEventListener("pagehide", persistAllProgress);
...
function formatSqlQuery(sql) {
  ...
  while ((match = regex.exec(sql)) !== null) {
    ...
    if (token.startsWith("'") || token.startsWith('"') || token.startsWith("`") || token.startsWith("--") || token.startsWith("/*")) {
      ...
      const kw = keywords.find((k) => k === upper);
      parts.push(kw ? kw : token);
    }
  }
...
function formatEditorCode() {
  ...
  if (isSql) {
    const raw = getEditorCode();
    const formatted = formatSqlQuery(raw);
...
async function checkCompilerStatus() {
  try {
    ...
    if (res.ok) {
      ...
      if (data.status) {
        ...
        const curProb = typeof PROBLEMS !== "undefined" && currentProblemId ? PROBLEMS[currentProblemId] : null;
        const isSql = curProb && curProb.language === "sql";
        compilerStatusText.textContent = isSql ? "MySQL Engine Ready" : "Java 21 Ready (Local)";
        appendLog(
          `[SYSTEM] Local Java & SQL Execution Engines detected:\n  - javac: ${data.javac_version}\n  - java:  ${data.java_version}\n  - sqlite: ${data.sqlite_version || "ready"}`,
        );
        return;
      }
    }
  ...
  const curProb = typeof PROBLEMS !== "undefined" && currentProblemId ? PROBLEMS[currentProblemId] : null;
  const isSql = curProb && curProb.language === "sql";
  compilerStatusText.textContent = isSql ? "MySQL Engine Ready" : "Online Compiler Active";
...
async function runAllTests() {
  ...
  if (!sourceCode) {
    showToast(isSql ? "Please enter a SQL query first" : "Please enter some Java code first");
    return;
  }
  ...
  appendLog(
    `\n=========================================\n[${isSql ? "SQL RUN" : "BUILD"}] Question: ${prob.title}\n${isSql ? "Executing SQL against SQLite database..." : "Compiling & Running test cases..."}\nTimestamp: ${new Date().toLocaleTimeString()}`,
  );
...
function handleExecutionResult(data, prob) {
  ...
  if (!data.compileSucceeded) {
    ...
    summaryCard.className = "summary-card compile-error";
    summaryTitle.textContent = isSql ? "SQL Execution Error" : "Compilation Error";
    summarySubtitle.textContent =
      data.compileOutput || (isSql ? "SQL execution failed." : "Check compiler log tab for details.");

    // Surface compiler error diagnostics directly in code editor (if Java)
    if (
      !isSql &&
    ...
    showToast(isSql ? "SQL execution failed - see logs" : "Compilation failed - see logs");
  ...
  if (passed === total) {
    ...
    summarySubtitle.textContent = `All ${passed} of ${total} test cases succeeded in ${data.durationMs}ms compiler runtime.`;
    diffDetailsSection.classList.add("hidden");
...
async function runCustomTestcase() {
  ...
  const sourceCode = getEditorCode().trim();
  if (!sourceCode) {
    showToast(isSql ? "Please enter a SQL query in the editor" : "Please enter Java code in the editor");
    return;
  }
  ...
  customResExpected.textContent = isSql ? "Executing SQL statement..." : "Comparing against logic...";
  customResTime.textContent = "";

  try {
    const seed = prob && prob.sampleCases && prob.sampleCases[0] ? prob.sampleCases[0].seedSql : "";
    const response = await fetch("./api/run", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        sourceCode: sourceCode,
        testCases: [{ id: 999, input: customVal, expected: "", seedSql: seed }],
        language: isSql ? "sql" : "java",
        problemId: prob ? prob.id : null,
        schemaSql: prob ? prob.schemaSql || "" : "",
      }),
    });
    ...
    if (response.ok) {
      ...
      if (!resData.compileSucceeded) {
        customResStdout.textContent = isSql ? "SQL Execution Error" : "Compilation Error";
        ...
        customResStatusBanner.textContent = isSql ? "SQL execution failed. Check logs." : "Compilation failed. Check logs.";
