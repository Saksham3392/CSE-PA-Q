...
# ==============================================================================
# Built-in High Performance SQLite Execution Engine for SQL Challenges (Q59-Q70)
# ==============================================================================
SQL_SCHEMAS = {
    "q59_mysql_ddl_create_table_students": "CREATE TABLE Students (\n    student_id INT PRIMARY KEY,\n    student_name VARCHAR(100) NOT NULL,\n    email VARCHAR(100) UNIQUE,\n    age INT CHECK (age >= 17 AND age <= 60),\n    course VARCHAR(50)\n);",
    "q60_mysql_dml_update_salary_increment": "CREATE TABLE Employees (\n    employee_id INT PRIMARY KEY,\n    employee_name VARCHAR(100),\n    department VARCHAR(50),\n    salary DECIMAL(10,2)\n);",
    "q61_mysql_dml_delete_minimum_price": "CREATE TABLE Books (\n    book_id INT PRIMARY KEY,\n    title VARCHAR(100),\n    author VARCHAR(100),\n    price INT\n);",
    "q62_mysql_group_by_student_count": "CREATE TABLE Students (\n    student_id INT PRIMARY KEY,\n    student_name VARCHAR(100),\n    department VARCHAR(50)\n);",
    "q63_mysql_group_by_customer_order_sum": "CREATE TABLE Orders (\n    order_id INT PRIMARY KEY,\n    customer_id INT,\n    amount INT\n);",
    "q64_mysql_group_by_subject_avg": "CREATE TABLE ExamMarks (\n    student_id INT,\n    subject VARCHAR(50),\n    marks INT\n);",
    "q65_mysql_group_by_having_frequent_shoppers": "CREATE TABLE Orders (\n    order_id INT PRIMARY KEY,\n    customer_id INT,\n    amount INT\n);",
    "q66_mysql_subquery_salary_above_average": "CREATE TABLE Employees (\n    employee_id INT PRIMARY KEY,\n    employee_name VARCHAR(100),\n    salary INT\n);",
    "q67_mysql_inner_join_students_departments": "CREATE TABLE Students (\n    student_id INT PRIMARY KEY,\n    student_name VARCHAR(100),\n    department_id INT\n);\nCREATE TABLE Departments (\n    department_id INT PRIMARY KEY,\n    department_name VARCHAR(100)\n);",
    "q68_mysql_left_join_employees_departments": "CREATE TABLE Employees (\n    employee_id INT PRIMARY KEY,\n    employee_name VARCHAR(100),\n    department_id INT\n);\nCREATE TABLE Departments (\n    department_id INT PRIMARY KEY,\n    department_name VARCHAR(100)\n);",
    "q69_mysql_self_join_employees_managers": "CREATE TABLE Employees (\n    employee_id INT PRIMARY KEY,\n    employee_name VARCHAR(100),\n    manager_id INT\n);",
    "q70_mysql_subquery_max_price_products": "CREATE TABLE Products (\n    product_id INT PRIMARY KEY,\n    product_name VARCHAR(100),\n    price INT\n);"
}


def execute_sql_solution(source_code, test_cases, problem_id=None, schema_sql=None):
    """
    In-memory SQLite execution pipeline for SQL questions:
    1. Sets up an in-memory SQLite database per test case.
    2. Runs schema DDL & seed SQL for the test case.
    3. Runs the user's SQL query/statements.
    4. Compares normalized actual rows with expected rows.
    """
    total_start = time.perf_counter()
    results = []

    trimmed_code = (source_code or "").strip()
    if not trimmed_code:
        return {
            "compileSucceeded": False,
            "compileOutput": "Error: SQL query is empty. Please enter a valid SQL query.",
            "durationMs": 0,
            "passed": 0,
            "total": len(test_cases),
            "results": []
    ...
    # Security check for SQLite sandbox
    lower_code = trimmed_code.lower()
    for forbidden in ["attach", "detach", "vacuum", "load_extension"]:
        if forbidden in lower_code:
            return {
                "compileSucceeded": False,
                "compileOutput": f"Security Error: '{forbidden}' statement is not allowed in sandbox.",
                "durationMs": 0,
                "passed": 0,
                "total": len(test_cases),
                "results": []
    ...
    # Fallback schema lookup by problem_id
    effective_schema = schema_sql or (SQL_SCHEMAS.get(problem_id) if problem_id else None)

    for tc in test_cases:
        tc_id = tc.get("id", 1)
        tc_input = str(tc.get("input", ""))
        tc_expected = str(tc.get("expected", "")).strip()
        tc_seed = tc.get("seedSql") or tc.get("seed") or ""
        tc_schema = tc.get("schemaSql") or effective_schema or ""
        ...
        try:
            ...
            # Clean user statements
            raw_stmts = [s.strip() for s in trimmed_code.split(";") if s.strip()]

            # Detect DDL (CREATE TABLE)
            has_create_table = any("create table" in s.lower() for s in raw_stmts)
            has_select = any(s.lower().startswith("select") or " select " in (" " + s.lower() + " ") for s in raw_stmts)

            if has_create_table:
                # Run DDL statement(s) first
                ddl_stmts = [s for s in raw_stmts if "create table" in s.lower()]
                for s in ddl_stmts:
                    cursor.execute(s)
                # Seed test data
                if tc_seed:
                    cursor.executescript(tc_seed)
                # Run remaining statements
                other_stmts = [s for s in raw_stmts if "create table" not in s.lower()]
                rows = []
                for s in other_stmts:
                    cursor.execute(s)
                    if cursor.description is not None:
                        rows = cursor.fetchall()
                # If user didn't write a select, auto-select from Students
                if not has_select:
                ...
                # 1. Run schema DDL
                if tc_schema:
                    cursor.executescript(tc_schema)
                # 2. Run seed SQL
                if tc_seed:
                    cursor.executescript(tc_seed)
                # 3. Run user statements
                rows = []
                for s in raw_stmts:
                    cursor.execute(s)
                    if cursor.description is not None:
            ...
            actual_clean = "\n".join(l.strip() for l in output_lines if l.strip())
            expected_clean = "\n".join(l.strip() for l in tc_expected.splitlines() if l.strip())
            ...
            def normalize_tokens(text):
                ...
                for line in text.strip().splitlines():
                    ...
                    for t in tokens:
                        try:
                            f = float(t)
                            norm_tokens.append(f"{f:.2f}")
                        except ValueError:
                            norm_tokens.append(t.lower())
                    norm_lines.append(" ".join(norm_tokens))
                return "\n".join(norm_lines)

            is_passed = (actual_clean.lower() == expected_clean.lower()) or (normalize_tokens(actual_clean) == normalize_tokens(expected_clean))
            duration_ms = round((time.perf_counter() - tc_start) * 1000, 2)

            results.append({
                "id": tc_id,
                "input": tc_input,
                "expected": tc_expected,
                "actual": actual_clean or "(empty result)",
                "status": "passed" if is_passed else "failed",
                "passed": is_passed,
                "durationMs": duration_ms,
                "error": None
    ...
    has_syntax_error = all(r.get("status") == "error" for r in results) if results else False
    error_msg = results[0]["error"] if (has_syntax_error and results) else "SQL Query executed successfully."

    return {
        "compileSucceeded": not has_syntax_error,
        "compileOutput": error_msg,
        "durationMs": total_duration_ms,
        "passed": passed_count,
        "total": len(test_cases),
        "results": results
...
def run_code():
    try:
        ...
        problem_id = str(payload.get("problemId", "")).lower()
        schema_sql = payload.get("schemaSql")
        ...
        # Route to SQL engine if SQL language, SQL question id, or SQL testcases detected
        is_sql = (
            language == "sql" or
            any(f"q{n}_" in problem_id for n in range(59, 71)) or
            (isinstance(test_cases, list) and len(test_cases) > 0 and isinstance(test_cases[0], dict) and ("seedSql" in test_cases[0] or "schemaSql" in test_cases[0]))
        )

        if is_sql:
            result = execute_sql_solution(source_code, test_cases, problem_id=problem_id, schema_sql=schema_sql)
